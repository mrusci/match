...
class ClusterOnnxDigitalRequantRewriter(DFPatternCallback):
...
class ClusterOnnxRequantTransform:
    ...
    def transform_module(
        ...
        for global_var, func in mod.functions.items():
            func = rewrite(ClusterOnnxDigitalRequantRewriter(), func)
...
class ClusterOnnxIntegerize(ExprMutator):
...
class ClusterFlattenRewriter(DFPatternCallback):
    """Rewriter for digital requant pattern
    """
    def __init__(self, require_type=False):
        super().__init__(require_type)
...
class BatchFlattenTransform:
    ...
    def transform_module(
        ...
        for global_var, func in mod.functions.items():
            func = rewrite(ClusterFlattenRewriter(), func)
            mod.update_func(global_var, func)
        return mod
...
def network_transformations(opts):
    ...
    pipeline.append(ClusterOnnxIntegerize('uint8'))
    return pipeline

def adjust_network(opts):
    pipeline=[]
    pipeline.append(LayoutTransform())
    pipeline.append(BatchFlattenTransform())
