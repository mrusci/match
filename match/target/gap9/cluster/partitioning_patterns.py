...
def dense_out_pattern():
    dense = is_op("nn.dense")(wildcard(), wildcard())
    add = is_op("add")(dense, is_constant()) | is_op("add")(
        is_op("cast")(dense), is_constant()
    )
    return add
...
def _biasadd_requant_pattern(linear_op):
    ...
    bias_add = is_op("nn.bias_add")(linear_op, wildcard()) | is_op("add")(
        linear_op, wildcard()
    )
...
def conv2d_pattern():
    ...
    conv2d = is_op("nn.dense")(wildcard(), wildcard())
...
def fully_connected_pattern():
    ...
    fc = is_op("nn.dense")(wildcard(), wildcard())
...
def _check_requant(pattern):
    ...
    if shift_factor < 0 or shift_factor > 31:
        logger.warning(
            "shift factor of accelerator operation must be in range [0, 31], but got {shift_factor}. Acceleration for this op is not supported."
        )
        return None
...
def _check_biasadd_requant(pattern):
    ...
    if str(right_shift_input.op.name) not in ["nn.bias_add", "add"]:
        logger.warning(
            "Found conv/dense op without nn.bias_add. Acceleration for this op is not supported."
        )
        return None
    ...
    if bias_dtype != "int32":
        logger.warning(
            f"Expected nn.bias_add parameters to be of type int32, but got {bias_dtype}. Acceleration for this op is not supported."
        )
        return None
...
def check_conv2d(pattern):
    ...
    def is_conv2d_attr_value_supported(attrs, name, supported_values):
        ...
        if attr not in supported_values:
            logger.warning(
                f"Expected nn.conv2d {name} to be one of {supported_values}, but got {attr}. "
    ...
    def is_filter_and_padding_supported(attrs):
        ...
        if (kernel_h not in supported_kernels) or (kernel_w not in supported_kernels):
            logger.warning(
                f"Expected nn.conv2d kernel width and height to be one of {supported_kernels}, "
        ...
        if (padding[0] != padding[2]) or (padding[1] != padding[3]):
            logger.warning(
                f"Expected equal top and bottom padding, and equal left and right padding,"
        ...
        if (kernel_w - 2 * padding[1] != 1) and (kernel_h - 2 * padding[0] != 1):
            ...
            logger.warning(
                f"Accelerator only supports 'SAME' padding. "
    ...
    if (
        not is_filter_and_padding_supported(conv2d.attrs)
        ...
        or not is_conv2d_attr_value_supported(
            conv2d.attrs, "groups", [1, num_output_channels]
        )
    ...
    if not weights_dtype.startswith("int"):
        logger.warning(
            f"Expected Conv2D weights to be of integer type, got {weights_dtype}. \
...
def check_element_wise_add(pattern):
    ...
    if tensor_shape_a != tensor_shape_b:
        logger.warning(
            f"Tensor shapes for element-wise-add don't match:"
...
def partitioning_patterns():
    return [
        PartitioningPattern(
            name="conv2d_bnorm_requant",
            pattern=conv2d_bnorm_requant_pattern,
            ordered_operation="nn.conv2d",
        ),
        PartitioningPattern(
            name="conv2d_bias_add_requant",
            pattern=conv2d_pattern,
            additional_checks=check_conv2d,
            ordered_operation="nn.conv2d",
        ),
        PartitioningPattern(
            name="conv2d_bias_add",
            pattern=only_conv_2d_and_bias_pattern,
            ordered_operation="nn.conv2d",
        ),
        PartitioningPattern(
            name="dense_bnorm_requant",
            pattern=dense_bnorm_requant_pattern,
            ordered_operation="nn.dense",
        ),
        PartitioningPattern(
            name="dense_bias_add_requant",
            pattern=fully_connected_pattern,
            additional_checks=check_fully_connected,
            ordered_operation="nn.dense",
        ),
        PartitioningPattern(
            name="add_requant",
            pattern=element_wise_add_pattern,
            additional_checks=check_element_wise_add,
            ordered_operation="add",
        ),
        PartitioningPattern(
            name="dense_out", pattern=dense_out_pattern, ordered_operation="dense"
        ),
    ]
